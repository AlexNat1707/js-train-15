...
function roundObjectValues(obj) {
  ...
  const objNew = Object.fromEntries(mappedArr);
  return objNew;
...
function calculateVolumeCylinder(radius, height) {
  ...
  const volume = Math.ceil(cylinderVolume);
  return volume;
...
function getFractionalPart(num) {
  ...
  const floatFractionalPart = parseFloat(fractionalPart.toFixed(8));
...
function estimateInvestment(principal, interestRate, years) {
  ...
  const sumOfInvestment = Math.round(principal * (1 + interestRate) ** years);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let initialValue = 0;
  let totalPrice = products.reduce((accumulator, product) => accumulator + product.price, initialValue);
  const result = Math.fround(totalPrice) > Math.fround(maxPrice);
  return result;
